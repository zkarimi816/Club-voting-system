...
public class BookResultsController implements Initializable{
	@FXML
    private AnchorPane bookresults;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select president, count(president) as result from bookvoter where club = 'Book Club' group by president having count(president)>=1");
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select vice_president, count(vice_president) as result from bookvoter where club = 'Book Club' group by vice_president having count(vice_president)>=1");
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select secretary, count(secretary) as result from bookvoter where club = 'Book Club' group by secretary having count(secretary)>=1");
	...
	private void setNode(Node node) {
		bookresults.getChildren().add((Node) node);
