...
public class candidatepagecontrol implements Initializable {
	
	ObservableList <String > positionlist = FXCollections.observableArrayList("President", "Vice President","Secretary");
	ObservableList <String > clublist = FXCollections.observableArrayList("Debate Club");

	    @FXML
	    private AnchorPane Candidateform;
	    ...
	    private ObservableList <CTableModal> data3 ;
	    ...
	    /* @FXML
	    public void viewallbutton(ActionEvent ev) {
	    	String id=  candidate_id.getText();
			try { 
				Connection mycon = DriverManager.getConnection(url,user,pass);
				Statement mystat = mycon.createStatement();
				data = FXCollections.observableArrayList();
				ResultSet rs = mystat.executeQuery("select * from candidates where club = 'Debate Club' and gpa >= 3.0 and id= '"+ id +"'");
				
				while(rs.next()) {
					data.add(new CTableModal(rs.getString("id"),rs.getString("full_name"),rs.getString("age"),
							rs.getString("major"),rs.getString("gpa"),rs.getString("email"),rs.getString("status"),
	    ...
	    private void updatebutton(ActionEvent event) {
			  ...
			  try { 
					...
					while(rs.next()) {
						data3.add(new CTableModal(rs.getInt("id"),rs.getString("full_name"),rs.getInt("age"),
	    ...
	    //setting values for combo boxes 
	    ...
	    private void setNode(Node node) {
	    	Candidateform.getChildren().add((Node) node);
	  	...
	  	public void all(ActionEvent eve) {
	  		try {
	    		Parent root = FXMLLoader.load(getClass().getResource("/application/allcandidatesdebate.fxml"));
