...
public class MusicCandidateControllar implements Initializable {
	
	ObservableList <String > positionlist = FXCollections.observableArrayList("President", "Vice President","Secretary");

	ObservableList <String > clublist = FXCollections.observableArrayList("Music Club");
	    @FXML
	    private AnchorPane Musiccandidateform;
		...
		if(!Gpa.equals("2.9")&&!Gpa.equals("2.8")&&!Gpa.equals("2.7")&&!Gpa.equals("2.6")
			...
			try { 
				...
				ResultSet rs = mystat.executeQuery("select * from candidates where club = 'Music Club' and gpa >= 3.0 and id= '"+ ID +"'");
...
*/
		 ...
		 private void setNode(Node node) {
		    	Musiccandidateform.getChildren().add((Node) node);
		 ...
		 public void all(ActionEvent eve) {
		  		try {
		    		Parent root = FXMLLoader.load(getClass().getResource("/application/allcandidatesmusic.fxml"));
