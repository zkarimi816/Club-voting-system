...
public class voter3 implements Initializable{

    @FXML
    private AnchorPane voter3;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		try { 
				...
				ResultSet rs = mystat.executeQuery("select * from speechvoter");				
				while(rs.next()) {
...
public void counttt(ActionEvent eve) {
   		...
   		try { 
   			Connection mycon = DriverManager.getConnection(url,user,pass);
   			Statement mystat = mycon.createStatement();
   			ResultSet rs = mystat.executeQuery("select count(full_name) as result from speechvoter");
   			while(rs.next()) {
   		...
   		public void delete(ActionEvent event) {
   	    	...
   	    	try { 
   				Connection mycon = DriverManager.getConnection(url,user,pass);
   				Statement mystat = mycon.createStatement();
   				mystat.executeUpdate("delete from speechvoter where full_name = ?");
