...
public class MusicVoterController {
	@FXML
    private AnchorPane MusicVoter;
    ...
    String url = "jdbc:mysql://localhost:3306/vote";
  	...
  	public void initialize() {
  		...
  		try{
	  		...
	  		ResultSet myres= mystat.executeQuery("select distinct(club) from candidates where club = 'Music Club'");
	  		while(myres.next()) {
  		...
  		try{
	  	 	Connection mycon= DriverManager.getConnection(url,user,pass);
	  		Statement mystat = mycon.createStatement();
	  		ResultSet myres= mystat.executeQuery("select full_name from candidates where position = 'President' and club = 'Music Club' and gpa>=3.0");
	  		while(myres.next()) {
		...
		try{
	  	 	Connection mycon= DriverManager.getConnection(url,user,pass);
	  		Statement mystat = mycon.createStatement();
	  		ResultSet myres= mystat.executeQuery("select full_name from candidates where position = 'Secretary' and club = 'Music Club' and gpa>=3.0");
	  		while(myres.next()) {
	  			String s = myres.getString("full_name");
		...
		try{
	  	 	Connection mycon= DriverManager.getConnection(url,user,pass);
	  		Statement mystat = mycon.createStatement();
	  		ResultSet myres= mystat.executeQuery("select full_name from candidates where position = 'Vice President' and club = 'Music Club' and gpa>=3.0");
	  		while(myres.next()) {
	  			String s = myres.getString("full_name");
	 ...
	 secretary.setText(secretarylist.getValue());
  	...
  	public void voterButton(ActionEvent event) {
    	...
    	try { 
			Connection mycon = DriverManager.getConnection(url,user,pass);
			Statement mystat = mycon.createStatement();
			mystat.executeUpdate("insert into musicvoter (voter_num,full_name,club,president,vice_president,secretary) values('"
  	...
  	private void setNode(Node node) {
  		MusicVoter.getChildren().add((Node) node);
