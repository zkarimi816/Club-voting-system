...
public class CSCCandidateControllar implements Initializable {

	ObservableList <String > positionlist = FXCollections.observableArrayList("President", "Vice President","Secretary");
	ObservableList <String > clublist = FXCollections.observableArrayList("Computer Science Club");
	@FXML
    private AnchorPane CSCCandidateform;
    ...
    @FXML
    private ComboBox<String> clubchoice;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
    	...
    	clubchoice.setValue("Select a Club");
    	clubchoice.setItems(clublist);
...
//setting values for choice boxes 
    ...
    public void candidate(ActionEvent event) {
    	...
    	String Club = clubchoice.getValue();
	...
	if(!Gpa.equals("2.9")&&!Gpa.equals("2.8")&&!Gpa.equals("2.7")&&!Gpa.equals("2.6")
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select * from candidates where club = 'Computer Science Club' and gpa >= 3.0 and id= '"+ ID +"'");
  ...
  /*  public void reset() {
    ...
    private void updatebutton(ActionEvent event) {
    	  // TABLE.getItems().get(TABLE.getSelectionModel().getSelectedIndex());	    	   
	    	...
	    	String Clubb = clubchoice.getValue();
		  try { 
    			Connection mycon = DriverManager.getConnection(url,user,pass);
    			Statement mystat = mycon.createStatement();
 	   ...
 	   TABLE.setOnMouseClicked(new EventHandler <MouseEvent>() {
 		   ...
 		   public void handle(MouseEvent event){
			   ...
			   candidate_id.setText(String.valueOf(t1.getid()));
 			   ...
 			   clubchoice.setValue(t1.getclub());
    ...
    private void setNode(Node node) {
		  CSCCandidateform.getChildren().add((Node) node);
	...
	public void all(ActionEvent eve) {
	  		try {
	    		Parent root = FXMLLoader.load(getClass().getResource("/application/allcandidatescsc.fxml"));
