...
public class MusicResultsController implements Initializable{
	@FXML
    private AnchorPane musicresults;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select president, count(president) as result from musicvoter where club = 'Music Club' group by president having count(president)>=1");
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select vice_president, count(vice_president) as result from musicvoter where club = 'Music Club' group by vice_president having count(vice_president)>=1");
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select secretary, count(secretary) as result from musicvoter where club = 'Music Club' group by secretary having count(secretary)>=1");
	...
	private void setNode(Node node) {
		musicresults.getChildren().add((Node) node);
