...
public class BookCandidateControllar  implements Initializable  {
	
	ObservableList <String > positionlist = FXCollections.observableArrayList("President", "Vice President","Secretary");
	ObservableList <String > clublist = FXCollections.observableArrayList("Book Club");
	@FXML
    private AnchorPane BookCandidateform;
	...
	if(!Gpa.equals("2.9")&&!Gpa.equals("2.8")&&!Gpa.equals("2.7")&&!Gpa.equals("2.6")
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select * from candidates where club = 'Book Club' and gpa >= 3.0 and id= '"+ ID +"'");
	   ...
	   /* @FXML
	    ...
	    private void setNode(Node node) {
	    	BookCandidateform.getChildren().add((Node) node);
	  	...
	  	public void all(ActionEvent eve) {
	  		try {
	    		Parent root = FXMLLoader.load(getClass().getResource("/application/allcandidatesbook.fxml"));
