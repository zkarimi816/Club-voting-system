...
public class SpeechCandidateControllar implements Initializable {

	ObservableList <String > positionlist = FXCollections.observableArrayList("President", "Vice President","Secretary");
	ObservableList <String > clublist = FXCollections.observableArrayList("Speech Club");
		@FXML
	    private AnchorPane SpeechCandidateform;
		...
		if(!Gpa.equals("2.9")&&!Gpa.equals("2.8")&&!Gpa.equals("2.7")&&!Gpa.equals("2.6")
			...
			try { 
				...
				ResultSet rs = mystat.executeQuery("select * from candidates where club = 'Speech Club' and gpa >= 3.0 and id= '"+ ID +"'");
	  ...
	  /*  public void reset() {
		...
		junior.setSelected(false);
		choiceposition.setValue("Select a position");
		choiceclub.setValue("Select a position");
	}*/
	   ...
	   /* @FXML
	    ...
	    private void setNode(Node node) {
	    	SpeechCandidateform.getChildren().add((Node) node);
	  	...
	  	public void all(ActionEvent eve) {
	  		try {
	    		Parent root = FXMLLoader.load(getClass().getResource("/application/allcandidatesspeech.fxml"));
