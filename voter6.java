...
public class voter6 implements Initializable {

    @FXML
    private AnchorPane voter6;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		try { 
				...
				ResultSet rs = mystat.executeQuery("select * from bookvoter");				
				while(rs.next()) {
...
public void counttt(ActionEvent eve) {
   		...
   		try { 
   			Connection mycon = DriverManager.getConnection(url,user,pass);
   			Statement mystat = mycon.createStatement();
   			ResultSet rs = mystat.executeQuery("select count(full_name) as result from bookvoter");
   			while(rs.next()) {
   		...
   		public void delete(ActionEvent event) {
   	    	...
   	    	try { 
   				Connection mycon = DriverManager.getConnection(url,user,pass);
   				Statement mystat = mycon.createStatement();
   				mystat.executeUpdate("delete from bookvoter where full_name = ?");
