...
public class SpeechResultsController implements Initializable {
	@FXML
    private AnchorPane speechresults;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select president, count(president) as result from speechvoter where club = 'Speech Club' group by president having count(president)>=1");
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select vice_president, count(vice_president) as result from speechvoter where club = 'Speech Club' group by vice_president having count(vice_president)>=1");
		...
		try { 
			...
			ResultSet rs = mystat.executeQuery("select secretary, count(secretary) as result from speechvoter where club = 'Speech Club' group by secretary having count(secretary)>=1");
	...
	private void setNode(Node node) {
		speechresults.getChildren().add((Node) node);
